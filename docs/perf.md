...
| Var                                      | Effect                                                                                                                                                                                                                                                                                                                                     |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ~~`MLX_NO_COMPILE=1`~~                   | **REMOVED (dead).** The Qwen3.5 compiled C++ forward path this var gated was deleted in the chat-engine refactor (`ee88b92b`); Qwen3.5 decode is now pure-Rust eager (per-op graph build). Nothing reads this var today.                                                                                                                   |
| ~~`MLX_EVAL_ALL_CACHES=1`~~              | **REMOVED (dead).** Token-only eval is the only strategy; nothing reads this var today.                                                                                                                                                                                                                                                    |
...
| `MLX_DISABLE_QWEN35_GDN_PREPARE=1`       | Disable the fully-fused `gdn_prepare` dispatch (window conv + SiLU + q                                                                                                                                                                                                                                                                     | k   | v split + q/k L2-norm + decay/beta gating in one kernel, decode/verify only). Fallback is the window_conv + split + norm + gating chain. ~1 bf16 ULP differences (norm eps placement, f32 beta); A/B and rollback. |
...
| `MLX_PAGED_NATIVE_KV_WRITE=0`           | Disable the default-on graph-native paged K/V write for every paged family. Whole-turn decode falls back to the synchronous write (also taken automatically on a native-write error). Batched and ragged decode instead **hard-error**, so this switch turns continuous batching off for every paged family                                                                                                                                                                                       |
| `MLX_PAGED_GRAPH_DECODE_GATHER=0`       | Disable the default-on graph-native paged decode gather for every paged family. Whole-turn decode falls back to the synchronous gather (also taken automatically when the graph gather cannot serve the inputs). LFM2 and Nemotron-H batched decode instead **hard-error**                                                                                                                                                                                                                        |
...
| `MLX_PAGED_GROUPED_D512=0\|force`       | Disable the direct-read BF16 D512/BS16 kernel or force a supported geometry for diagnostics. Ordinary single-row Gemma AR learns its route from completed tokens; other paths retain their existing policy. Metal pipeline and shape guards always apply.                                                                                                                                                                                                                                         |
| `MLX_PAGED_GROUPED_D512_STRIPES`        | Diagnostic partition override (`4\|8\|16\|32\|64\|128\|256`). It does not enable grouped attention by itself; pair with `MLX_PAGED_GROUPED_D512=force` when testing an unmeasured geometry. Automatic Gemma AR selects partitions on the current device.                                                                                                                                                                                                                                          |
| `MLX_GEMMA4_PAGED_DECODE_ROUTE`         | Diagnostic Gemma decode route (`auto\|sdpa\|paged`). Auto uses the learned grouped choice when available; other routes keep their geometry and live-memory guards. An explicit value disables attention-route learning.                                                                                                                                                                                                                                                                           |
| `MLX_GEMMA4_DECODE_TUNING=0`            | Disable device-local Gemma AR calibration at process startup. By default it learns grouped-D512 partitions and early-submission depth from completed real tokens, cached per loaded model and context scale. It adds no forward passes or GPU waits.                                                                                                                                                                                                                                              |
...
Gemma calibration is a bounded search over supported shapes and kernel parameters,
...
Qwen3.5 / Qwen3.6 MTP (Multi-Token Prediction) speculative decoding adds
runtime knobs gating individual optimizations across the W6.5–Phase C perf
chain (the former "one unconditional warmup hook for verify prewarm" was
deleted with the compiled-C++ MTP path — nothing pre-warms the eager verify
at model load). The env vars are read at most once per process and cached;
the truthy/falsy vocabulary is uniform (`1` / `true` / `on` and `0` /
`false` / `off`, case-insensitive, with `trim()`) everywhere except
`MLX_NEMOTRON_MTP_DEFAULT`, which matches the literal string `1` and
nothing else. The last row of the table is a per-family DEFAULT policy
switch rather than an optimization knob. The adaptive-depth knob
is a TypeScript `ChatConfig` field (not an env var) because it interacts
with
the user-set `mtpDepth` and needs per-session resolution.

| Knob                          | Default              | Workstream | Direction         | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------- | -------------------- | ---------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (eager verify prewarm)        | ~~always~~           | W6.7       | ~~unconditional~~ | **REMOVED.** The once-per-process 10-dummy-shape prewarm was deleted with the compiled-C++ MTP path (`ee88b92b`); the verify forward is now eager Rust, re-traced per cycle, and nothing warms it at model load.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `MLX_MTP_BUCKETED_VERIFY`     | ~~ON~~               | W6.29      | ~~opt-OUT~~       | **REMOVED (dead).** Per-bucket compiled verify graphs were deleted with the compiled-C++ MTP path (`ee88b92b`); nothing reads this var today. The measured long-decode win it documented (AR +12.0%, MTP +26.1% on qwen3.5-4b / M3 Max) is currently **not implemented** — the verify forward re-traces eagerly per cycle at whatever kv_len exists. Restoring per-kv-len compiled verify graphs is an open perf follow-up.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `MLX_MTP_ACCEPT_GATE`         | ON                   | —          | opt-OUT           | MTP acceptance gate, **depth-1-scoped, adaptive-exempt, and exact-binomial**. When ON (default), the model aggregates the first-draft acceptance counts (accepted/attempted at draft slot 0) across completed fixed depth-1 turns and disables speculation for the NEXT fixed depth-1 turn only when an exact-binomial test at the 5% level shows the aggregate is inconsistent with the `0.6` break-even — plain AR — so a weak draft head does not pay the verify cost for zero speedup. The exact test preserves a uniform false-gate bound at every sample size: a 1-cycle turn, a 2-of-4 streak from a healthy 0.756 head (~25% of turns), and 0-of-2 / 0-of-3 / 1-of-5 aggregates (~5.95% / 1.45% / 1.43% false rates) never gate; only extreme aggregates like 0-of-4 (false rate ~0.35%) do. The aggregate is bounded (halved past ~512 samples) so a long healthy phase cannot drown out a later degradation. Depth > 1 turns and `mtpAdaptiveDepth` turns (which sweep depths 1-5) are never gated and do not publish history. State is per-model; the first turn of a model load probes, after 3 consecutive gated turns the gate re-probes, and a full session reset (`reset_caches`) clears the history. Measured first-draft acceptance on real checkpoints: 1.000 (qwen3.5-4b, counting), 0.756 (qwen3.5-4b, complex task), 0.000 (qwen3.5-0.8b — MTP is a ~0.6× net loss there). Set to `0` / `false` / `off` to always run MTP when requested. |
| `MLX_NEMOTRON_MTP_DEFAULT`    | OFF                  | —          | opt-IN            | **NemotronH only, and a default-policy switch rather than a perf knob.** It gates `NemotronHModel::mtp_auto_enabled()` (`models/nemotron_h/model.rs`), the native getter `ChatSession` consults before auto-setting `enableMtp` for a caller who set nothing. Unset (the default) the getter returns `false`, so the family never auto-enables MTP even though `hasMtpWeights()` is true on every shipped checkpoint; `=1` makes it return `true`, but only on a checkpoint that actually carries a head (the env read is ANDed with `mtp_active`). **The one knob here that is off the shared truthy vocabulary**: the read is `std::env::var("MLX_NEMOTRON_MTP_DEFAULT").is_ok_and(\|v\| v == "1")`, so `true`/`on`/`TRUE`do NOT enable it. Cached in a process-wide`OnceLock`at the first`mtpAutoEnabled()`call, so it is neither per-model nor changeable afterwards. It never blocks an explicit request —`enableMtp: true`or`generation_mode: "mtp"` runs MTP with the var unset. Why the default is off (and why the throughput argument for it was retracted): docs/models.md.                                                                                                                                                                                                                                                                                                                                                                          |
...
skip prompt-prefill (cache-reuse / VLM / delta / prompt < 2 tokens) the
cache simply starts empty and fills
...
- **We run 2** — a single-token **Step-A** main-model forward, gated by
  `do_step_a` (now `engine/mtp_turn.rs`, driven by the stepper's
  `forward_with_hidden`) — **plus** the `D+1` verify. The Step-A forward
  exists only to produce the next draft seed, which the verify forward
  already computes and we discard on the default path. (At the time of the
  original measurement this ran through the now-deleted
  `forward_compiled_with_hidden`; the eager `DenseMtpStepper` preserves the
  same 2-forward cycle structure.)
...
- Source of truth (env-var readers + inventory table):
  `crates/mlx-core/src/models/qwen3_5/mtp_decode.rs`
  (`mtp_chained_cycles_enabled`, `mtp_accept_gate_enabled`). The W6.29
  bucketed-verify dispatcher no longer exists — it was C++-side state in
  the deleted `mlx_qwen35.cpp`. The verify async-eval, deferred-hidden,
  sparse-accept, batched-target and prompt-prefill knobs are gone: each of
  those behaviors is now unconditional.
- Phase C committed-history MTP cache: eager policy in
  `crates/mlx-core/src/models/qwen3_5/model.rs`
  (`DenseMtpStepper::begin_cycle` / `commit_mtp`, `committed_len`); the old
  C++ policy file `mlx_qwen35_mtp_compiled.cpp` (`g_mtp_committed_len`) was
  deleted with the compiled path; prompt-prefill seed lives in
  `begin_mtp_decode`'s prompt-prefix seed block (via
